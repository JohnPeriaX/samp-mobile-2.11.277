...
#include "vendor/ini/INIReader.h"

#include "vendor/ini/SimpleIni.h"
...
CSettings::CSettings()
{
	...
	std::string settingsPath = BuildStoragePath("SAMP/settings.ini");
	const char* buff = settingsPath.c_str();
	...
	// client
	size_t length = 0;
	char fallbackName[0x7F];
	sprintf(fallbackName, "__android_%d%d", rand() % 1000, rand() % 1000);
	length = reader.Get("client", "name", fallbackName).copy(m_Settings.szNickName, 24);
